...
class Football_Pool_Admin_Bonus_Questions extends Football_Pool_Admin {
	...
	private static function view() {
		...
		$cols = array(
			...
			array( 'date', __( 'answer before', 'football-pool' ) . '<br><span style="font-size:80%">(' . _x( 'e.g.', 'abbreviation for "for example"', 'football-pool' ) . ' ' . self::example_date() . ')</span>', 'lastdate', ''),
			array( 'date', __( 'score date', 'football-pool' ).'<br><span style="font-size:80%">(' . _x( 'e.g.', 'abbreviation for "for example"', 'football-pool' ) . ' ' . self::example_date() . ')</span>', 'scoredate', '' ),
//			array( 'text', __( 'answer', 'football-pool' ), 'answer', '' ),
			array( 'integer', __( 'question order', 'football-pool' ), 'question_order', '' ),
			array( 'link', __( 'linked to match', 'football-pool' ), 'match', '' )
		);
