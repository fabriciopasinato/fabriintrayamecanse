...
if ( defined( 'FOOTBALLPOOL_CSV_DATE_FORMAT' ) ) {
	// Convert the dates to the non-standard date format that the user defined.
	$matches_alt_date_format = array_map( function( $m ) {
			$d = DateTime::createFromFormat( 'Y-m-d H:i:s', $m['play_date'] );
			$m['play_date'] = $d->format( FOOTBALLPOOL_CSV_DATE_FORMAT );
			return $m;
		}, $matches
	);

	$matches = $matches_alt_date_format;
}
