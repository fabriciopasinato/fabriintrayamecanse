...
class Football_Pool_Admin {
	...
	public static function admin_menu_init() {
		...
		do_action( 'footballpool_admin_pre_menu_init', $slug, FOOTBALLPOOL_ADMIN_BASE_CAPABILITY );
...
*/
		...
		do_action( 'footballpool_admin_post_menu_init', $slug, FOOTBALLPOOL_ADMIN_BASE_CAPABILITY );
	}
...
//									 'shortcodes' => implode( '|', array_map( array( 'self', 'get_shortcode_codes' ), $shortcodes ) ),
	...
	*/
	...
	public static function secondary_button( $text, $action, $wrap = false, $type = 'button', $other_attributes = null ) {
		if ( $type === 'button' ) {
			echo self::get_secondary_button( $text, $action, $wrap = false, $type = 'button', $other_attributes = null );
