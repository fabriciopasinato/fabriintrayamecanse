...
class Football_Pool_Admin_Users extends Football_Pool_Admin {
	...
	private static function view() {
		...
		$users = self::get_users( 
									...
									$search, $search_by, $league_id
								);
		
		$cols = array();
		$cols[] = array( 'text', __( 'name', 'football-pool' ), 'name', '' );
		if ( $has_leagues ) {
			$cols[] = array( 'select', __( 'plays in league', 'football-pool' ), 'plays_in_league', '' );
			$cols[] = array( 'text', __( 'registered for league', 'football-pool' ), 'subscribed_for_league', '' );
		} else {
			$cols[] = array( 'checkbox', __( 'not a user in the pool', 'football-pool' ), 'is_no_player', '' );
		}
		$cols[] = array( 'checkbox', __( 'paid?', 'football-pool' ), 'paid_for_pool', '' );
